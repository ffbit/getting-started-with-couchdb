  ...
  "shows": {
    "recipe": "function(doc, req) { 
      ...
      for (var i = 0; i < doc.ingredients.length; i++) {
        output += '<li>' + doc.ingredients[i].measure + ' ' + doc.ingredients[i].ingredient + '</li>';
      }
      output += '</ul>';

      output += '<h2>Method</h2>';
      output += '<ol>';
      for (var i = 0; i < doc.method.length; i++) {
        output += '<li>' + doc.method[i].text + '</li>';
      }
      output += '</ol>';
